...
$brevoConfig = Brevo\Client\Configuration::getDefaultConfiguration()
    ->setApiKey('api-key', $_ENV['BREVO_API_KEY']);

$brevoApiInstance = new Brevo\Client\Api\TransactionalEmailsApi(
...
try {
    ...
    if ($event->type === 'checkout.session.completed') {
        ...
        $email = new Brevo\Client\Model\SendSmtpEmail([
            ...
            'subject' => 'Conferma ordine #' . $orderData['order_id'],
            'htmlContent' => $htmlContent
        ]);

        try {
            $result = $brevoApiInstance->sendTransacEmail($email);
            error_log('Email inviata con successo a: ' . $orderData['customer_email']);
        } catch (Exception $e) {
            error_log('Errore nell\'invio email: ' . $e->getMessage());
        }

        http_response_code(200);
        echo 'Webhook processed successfully - Email sent';
    } else {
